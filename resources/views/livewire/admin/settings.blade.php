<div class="row">
    ...
    <script>
        function getMyLocation() {
            if (navigator.geolocation) {
                const success = (position) => {
                    ...
                    livewire.emit('setBaseLocation', lat, long);
                    livewire.emit('renderMap', lat, long);
                }
                const error = (error) => {
                    Swal.fire({
                        icon: 'error',
        ...
        function initMap(_lat, _long) {
            const lat = _lat || `{{$form['base_lat']}}`;
            const long = _long || `{{$form['base_long']}}`;
            ...
            mymap.on('click', function(e) {
                ...
                livewire.emit('setBaseLocation', e.latlng.lat, e.latlng.lng);
            });
        }
        ...
        $(document).ready(function() {
            ...
            livewire.on('renderMap', function(newLat, newLong, location) {
                $('#map-wrapper > #map').remove();
                $('#map-wrapper').append('<div id="map" wire:ignore></div>');
                console.log(newLat, newLong, location);
                initMap(newLat, newLong);
            });
            livewire.on('getMyLocation', function() {
                getMyLocation();
            });
        })
    </script>
